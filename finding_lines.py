...
def draw_lane(img, left_line, right_line, lane_color=(3, 40, 252), road_color=(252, 20, 3)):
    ...
    cv2.fillPoly(window_img, np.int32([left_pts]), lane_color)
    cv2.fillPoly(window_img, np.int32([right_pts]), lane_color)
    ...
    # Draw the lane onto the warped blank image
    cv2.fillPoly(window_img, np.int32([pts]), road_color)
...
def print_road_map(image, left_line, right_line):
    ...
    # Draw the lane onto the warped blank image
    cv2.fillPoly(window_img, np.int32([left_pts]), (140, 0, 170))
    cv2.fillPoly(window_img, np.int32([right_pts]), (140, 0, 170))
